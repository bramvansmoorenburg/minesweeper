...
```{r}
imageits=100 #one image every N iterations
fieldsource="fielderror.xlsx" #if you want to use a file, set the path here
allowerrors=TRUE
height=100
width=100
minesfrac=0.2
...
compute_neighb_prod <- function(mx) {
  ...
  sum_neighb_each <- function(x) {
    ...
    prod(mx[near.ind.val])      # Now product the surrounding cell values
  }
  `dim<-`(                     # this is just to return in same matrix format as input
    sapply(
      split(mx.ind, row(mx.ind)),   # For each x, y coordinate in input mx
...
writePNG(fieldimg,"Field Image 0.png")
it=0
repeat {
  repeat {
  it=it+1
  ...
  if ((it-1)%%imageits==0) {
    ...
    invisible(apply(indexmat,1,coloring))
    fieldimg=fieldimg/255
    name=paste0("Field Image ",it,".png")
  ...
  if (length(primebingo)>0) {
    ...
    fieldtiles=compute_neighb_sum(field==pi)
    fieldflags=compute_neighb_sum(field==exp(1)|field==10)
    cat("Executing Prime Sweep, iteration: ",it,", flags: ",sum(field==exp(1)),"/",totmines,", grey tiles: ",sum(field!=pi),"/",prod(dim(field)),", errors: ",sum(field==10),"\n",sep = "")
  }
  }
  
  fieldnums=field==trunc(field)&fieldtiles>0&field!=10
  if (all(fieldhist==field)&allowerrors&sum(field==pi)>0&sum(fieldnums)>0) { #randomly sweep tiles with lowest probability of containing a mine
    fieldsum=compute_neighb_sum(field)
    fieldmoves=fieldsum!=8*pi&fieldsum!=3*pi&fieldsum!=5*pi&field!=trunc(field)&field!=exp(1)
